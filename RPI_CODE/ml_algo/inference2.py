...
class AudioProcessor:
    def audio_predict(self):
        ...
        for i in range(HIST_SIZE):
            weights[i] = math.exp(-i/(5))
            self.prediction_queue.append([0]*521)
        weights /= np.linalg.norm(weights)
        weights = np.asarray(weights).reshape((HIST_SIZE,1))
        ...
        while(True):
            ...
            weighted_predictions = np.asarray(list(self.prediction_queue))
            weighted_predictions = np.sum(weighted_predictions * weights, axis = 0)
            ...
            if("Gunshot, gunfire" in top5_classes):
                ...
                if likely_fireworks == False:
                    if(max(weighted_predictions[FIREWORKS_VAL], weighted_predictions[FIRECRACKER_VAL]) > prediction[GUNSHOT_VAL]):
                        print("************GUNSHOT PREDICTION OVERRIDDEN BY FIREWORK CONTEXT***********")
                    else:
                        print("***************GUNSHOT PREDICTED*************")
                        prediction = prediction - prediction.mean(axis=0)
                        prediction = prediction / np.abs(prediction).max(axis=0)
                        self.prediction_value = (prediction[GUNSHOT_VAL], np.max(recording))
    ...
    def audio_process(self):
        ...
        timeout_counter = 0
        timeout_val = TIMEOUT * self.sample_rate
        counter = 0
            ...
            while(True):
                ...
                if(len(line) == 2):
                    ...
                    if(abs(val) > self.threshold):
                        ...
                        if(timeout_counter == timeout_val - 1):
                            self.prediction_queue.append([0]*521)
                        timeout_counter = (timeout_counter + 1) % timeout_val
    ...
    def __init__(self, th=2000, sr=16000, dur=1):
        ...
        self.prediction_queue = collections.deque(maxlen = HIST_SIZE)
